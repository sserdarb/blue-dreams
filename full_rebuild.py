...
build_cmd = f'''docker run --rm --name blue-dreams-builder \
    --network {net} \
    -e DATABASE_URL="postgresql://coolify:***REDACTED_DB_PASSWORD***@coolify-db:5432/blue_dreams_v2" \
    -w /app \
...
if bid and 'Error' not in bid and 'No such' not in bid:
    ...
    if 'Up' in out and 'Restarting' not in out:
        ...
        out, _ = run(f'docker exec -w /app -e DATABASE_URL="postgresql://coolify:***REDACTED_DB_PASSWORD***@coolify-db:5432/blue_dreams_v2" {CID} npx prisma db push --accept-data-loss 2>&1', t=60)
