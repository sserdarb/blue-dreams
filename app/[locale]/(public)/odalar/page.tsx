...
export default async function RoomsPage({ params }: { params: Promise<{ locale: string }> }) {
    ...
    const rooms: RoomRow[] = await prisma.room.findMany({
        where: { locale },
        orderBy: { order: 'asc' },
        select: {
            id: true,
        ...
        <div>
            ...
            {!hasRoomListWidget && (
                <>
                    ...
                    {!hasWidgets && rooms.length > 0 && (
                        <section className="relative h-[60vh] min-h-[400px] flex items-center justify-center overflow-hidden">
                            <div
                                className="absolute inset-0 bg-cover bg-center"
                                style={{ backgroundImage: `url(${rooms[1]?.image || rooms[0]?.image})` }}
                            />
                        ...
                        <div className="container mx-auto px-6">
                            ...
                            {rooms.length === 0 ? (
                                <div className="text-center py-12 text-gray-500">
                                    <p>Oda bilgileri yakında eklenecektir.</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                    {rooms.map(room => (
                                        <Link
                                            key={room.id}
                                            href={`/${locale}/odalar/${room.slug || room.id}`}
                                                ...
                                                <div>
                                                    ...
                                                    <div className="grid grid-cols-3 gap-3 mb-6">
                                                        ...
                                                        <div className="text-center p-3 bg-sand rounded-lg">
                                                            ...
                                                            <span className="block text-[10px] font-bold text-gray-900 line-clamp-2 leading-tight mt-1">{room.view}</span>
                                                        </div>
                                                    </div>
                                                </div>
