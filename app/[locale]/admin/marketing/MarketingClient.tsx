...
import { ArrowUpRight, ArrowDownRight, DollarSign, Target, MousePointer, Eye, FileDown } from 'lucide-react'
        ...
        <div ref={contentRef} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <Card className="p-4 bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-sm text-slate-500 dark:text-slate-400">ROAS</p>
                            <h3 className={`text-2xl font-bold ${overview.totalROAS >= 4 ? 'text-green-500 dark:text-green-400' : 'text-yellow-500 dark:text-yellow-400'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {overview.platformBreakdown.map(p => (
                    <Card key={p.platform} className="p-4 bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white">
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="border-slate-200 dark:border-slate-800 hover:bg-transparent">
                            <TableHead className="text-slate-500 dark:text-slate-400">Campaign</TableHead>
                            <TableHead className="text-slate-500 dark:text-slate-400">Platform</TableHead>
                            <TableHead className="text-slate-500 dark:text-slate-400">Status</TableHead>
                            <TableHead className="text-right text-slate-500 dark:text-slate-400">Spend</TableHead>
                            <TableHead className="text-right text-slate-500 dark:text-slate-400">Roas</TableHead>
                            <TableHead className="text-right text-slate-500 dark:text-slate-400">Conv.</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {campaigns.map((c) => (
                            <TableRow key={c.id} className="border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                ...
                                <TableCell>
                                    <Badge className={`${c.status === 'active' ? 'bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-400 hover:bg-green-200 dark:hover:bg-green-900/70' :
                                        'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800'
                                        }`}>
                                        {c.status}
                                    </Badge>
                                </TableCell>
                                ...
                                <TableCell className="text-right">
                                    <span className={`${c.roas >= 5 ? 'text-green-600 dark:text-green-400' : c.roas >= 2 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400'}`}>
                                        {c.roas.toFixed(1)}x
                                    </span>
                                </TableCell>
                                <TableCell className="text-right text-slate-700 dark:text-slate-300">
