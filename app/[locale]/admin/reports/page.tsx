...
export default async function ManagementReportsPage({
    ...
    try {
        ...
        console.error(`[Reports] Error loading ${group} reports:`, err)
        fetchError = (err as Error).message || 'Bilinmeyen hata'
    }
    ...
    return (
        <div className="space-y-6">
            ...
            {fetchError ? (
                <div className="rounded-xl border border-red-200 bg-red-50 dark:bg-red-900/20 dark:border-red-800 p-8 text-center">
                    <div className="text-4xl mb-4">⚠️</div>
                    <h2 className="text-xl font-semibold text-red-800 dark:text-red-300 mb-2">Rapor yüklenemedi</h2>
                    <p className="text-red-600 dark:text-red-400 mb-4">{fetchError}</p>
                    <a href={`?group=${group}`} className="inline-flex items-center px-4 py-2 bg-red-100 dark:bg-red-800 text-red-800 dark:text-red-200 rounded-lg hover:bg-red-200 dark:hover:bg-red-700 transition-colors">
                        🔄 Tekrar Dene
                    </a>
                </div>
            ) : content}
