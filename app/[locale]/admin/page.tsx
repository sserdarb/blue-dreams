    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-emerald-900/30 border border-emerald-500/30 rounded-lg px-4 py-2 flex items-center gap-2">
          ...
          <span className="text-emerald-300 text-sm">{dt.page.connectionStatus}</span>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">
                {dt.page.selectedPeriod}: <strong>{dateLabel}</strong>
              </p>
            </div>
          </div>
          <DashboardFilter />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
          <div className="bg-gradient-to-br from-cyan-600 to-blue-700 rounded-2xl p-6 text-white shadow-xl shadow-cyan-900/20">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-cyan-100 font-medium">{dt.page.totalReservations}</p>
                <p className="text-xs text-cyan-200">{dt.page.selectedPeriod}: {dateLabel}</p>
              </div>
            </div>
            <div className="flex items-end justify-between">
              <div>
                <h2 className="text-5xl font-black">{asisiaStats.totalReservations.toLocaleString('tr-TR')}</h2>
                <p className="text-cyan-100 mt-1">{dt.page.confirmedReservations}</p>
              </div>
              <div className="text-right">
                <p className="text-xl font-bold">{symbol}{asisiaStats.totalRevenue.toLocaleString('tr-TR', { maximumFractionDigits: 0 })}</p>
                <p className="text-sm font-medium text-cyan-200">{dt.page.totalGuests}: {asisiaStats.totalGuests}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-orange-500 to-rose-600 rounded-2xl p-6 text-white shadow-xl shadow-orange-900/20">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-orange-100 font-medium">{dt.page.adrTitle}</p>
                <p className="text-xs text-orange-200">{dateLabel} · {dt.page.revenuePerRoomNight}</p>
              </div>
            </div>
            <div className="flex items-end justify-between">
              <div>
                <h2 className="text-5xl font-black">{primaryAdrString}</h2>
                <p className="text-orange-100 mt-1">{dt.page.avgNightlyProfit}</p>
              </div>
              <div className="text-right">
                <p className="text-3xl font-bold">{secondaryAdrString}</p>
                <p className="text-sm font-medium text-orange-200">{dt.page.currentRate}: {tryRate.toFixed(2)} ₺</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-red-600 to-red-800 rounded-2xl p-6 text-white shadow-xl shadow-red-900/20">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-red-100 font-medium">{dt.page.cancelledReservations}</p>
                <p className="text-xs text-red-200">{dt.page.selectedPeriod}: {dateLabel}</p>
              </div>
            </div>
            <div className="flex items-end justify-between">
              <div>
                <h2 className="text-5xl font-black">{asisiaStats.cancelledReservations?.toLocaleString('tr-TR') || 0}</h2>
                <p className="text-red-100 mt-1">{dt.page.cancelCount}</p>
              </div>
              <div className="text-right">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-2 mb-6 relative group">
              ...
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">{dt.page.generalReservationTrend}</h2>
              ...
              <div className="absolute right-0 top-8 w-64 bg-slate-800 dark:bg-slate-700 text-white p-3 rounded-xl shadow-xl border border-slate-700 dark:border-slate-600 z-50 hidden group-hover:block transition-all text-xs space-y-2 pointer-events-none">
                <p className="font-semibold border-b border-slate-600 pb-1 mb-2">{dt.page.chartInfo}</p>
                <div className="text-slate-300">{dt.page.chartDescription}</div>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">{dt.page.otaTrends}</h2>
            </div>
          ...
          <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">{dt.page.callCenterTrends}</h2>
            </div>
          ...
          <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">{dt.page.onlineTrends}</h2>
            </div>
      ...
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 mt-1">{dt.page.welcomeMessage}</p>
          </div>
        </div>

        <ModuleOffline
          moduleName={dt.page.moduleOfflineName}
          dataSource="elektra"
          offlineReason={dt.page.moduleOfflineReason}
        />
      </div>
    )
  }
