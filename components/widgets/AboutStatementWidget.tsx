...
interface AboutStatementData {
    label?: string
    heading?: string
    headingParts?: { text: string; accent?: boolean }[]
}

export function AboutStatementWidget({ data }: { data: AboutStatementData }) {
    return (
                    ...
                    <h2 className="text-3xl md:text-5xl lg:text-6xl font-serif leading-tight md:leading-snug font-light">
                        {data.headingParts ? (
                            data.headingParts.map((part, i) =>
                                part.accent ? (
                                    <span key={i} className="italic font-normal text-brand-light">{part.text}</span>
                                ) : (
