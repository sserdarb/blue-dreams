...
c.connect('76.13.0.113', username='root', password="***REDACTED_SSH_PASSWORD***", timeout=10)
sftp = c.open_sftp()
...
for img in ['blue-dreams-build:latest', 'blue-dreams-final:latest', 'blue-dreams-rebuild:latest']:
...
files = [
    ...
    'app/api/ai/chat/route.ts', 'app/api/ai/settings/route.ts',
    'app/admin/ai-training/page.tsx', 'app/api/admin/users/route.ts',
    'app/admin/users/page.tsx', 'app/admin/login/page.tsx',
    'app/admin/layout.tsx', 'app/actions/auth.ts',
    'app/api/analytics/data/route.ts',
    'components/admin/widget-editors/widget-types.ts',
    'components/admin/widget-editors/index.tsx',
    'nixpacks.toml', 'prisma/seed.js',
]
for rel in files:
...
# Check BUILD_ID via docker cp (container is stopped after run)
...
# Step 5: Deploy
print("\n=== Step 5: Deploy ===")
# Escape backticks properly for shell
create_cmd = (
    f'docker run -d --name {CNAME} --network coolify '
    f'--restart unless-stopped '
    f'-e DATABASE_URL="{DB_URL}" '
    f'-e ADMIN_EMAIL="***REDACTED_EMAIL***" '
    f'-e ADMIN_PASSWORD="***REDACTED_ADMIN_PASSWORD***" '
