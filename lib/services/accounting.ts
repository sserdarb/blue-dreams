...
export const AccountingService = {
    ...
    async getStockList(): Promise<{ items: StockItem[]; source: 'live' | 'demo' }> {
        return erpCache.getOrFetch('accounting:stockList', async () => {
            try {
                ...
                console.log('[Accounting] FN_ACCOUNTING_GET_STOCK_LIST: no data or unexpected format')
            } catch (err) {
    ...
    async getAccountingReceipts(fromDate?: string, toDate?: string): Promise<{ receipts: AccountingReceipt[]; source: 'live' | 'demo' }> {
        const cacheKey = `accounting:receipts:${fromDate || 'all'}:${toDate || 'all'}`
            ...
            try {
                ...
                if (data && (Array.isArray(data) || data?.Result || data?.data)) {
                    ...
                    if (Array.isArray(raw) && raw.length > 0) {
                        ...
                        return { receipts, source: 'live' as const }
                    }
                }
    ...
    async getForecast(): Promise<{ forecast: ForecastDay[]; source: 'live' | 'demo' }> {
        return erpCache.getOrFetch('accounting:forecast', async () => {
            try {
                const now = new Date()
                ...
                if (data && (Array.isArray(data) || data?.Result || data?.data)) {
                    ...
                    if (Array.isArray(raw) && raw.length > 0) {
                        ...
                        return { forecast, source: 'live' as const }
                    }
                }
                console.log('[Accounting] Forecast: no data')
            } catch (err) {
    ...
    clearCache(): void {
        erpCache.invalidatePrefix('accounting:')
    }
}
