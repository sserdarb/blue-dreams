...
var FALLBACK_RATES = { EUR_TO_TRY: 1, USD_TO_TRY: 1, fetchedAt: 0 };
// ─── Country Cache ─────────────────────────────────────────────
...
function computeOccupancy(availability, dynamicTotalRooms) {
    var byDate = new Map();
    ...
    for (var _a = 0, byDate_1 = byDate; _a < byDate_1.length; _a++) {
        ...
        var effectiveTotal = dynamicTotalRooms || TOTAL_ROOMS;
...
exports.ElektraService = {
    ...
    getTotalRooms: function () {
        return __awaiter(this, void 0, void 0, function () {
            var today, nextWeek, from, to, avail, roomTypeTotals, _i, avail_1, item, key, totalFromApi;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        ...
                        return [4 /*yield*/, fetchAvailability(from, to)];
                    case 1:
                        avail = _a.sent();
                        if (!avail || avail.length === 0) {
                            console.warn('[Elektra] No availability data for getTotalRooms, using fallback:', TOTAL_ROOMS);
                            return [2 /*return*/, TOTAL_ROOMS];
                        }
                        roomTypeTotals = new Map();
                        for (_i = 0, avail_1 = avail; _i < avail_1.length; _i++) {
                            item = avail_1[_i];
                            if (SALEABLE_ROOM_CODES.includes(item.roomType)) {
                                key = item.roomType;
                                if (!roomTypeTotals.has(key)) {
                                    roomTypeTotals.set(key, item.availableCount);
                                } else {
                                    var current = roomTypeTotals.get(key);
                                    if (item.availableCount > current) {
                                        roomTypeTotals.set(key, item.availableCount);
                                    }
                                }
                            }
                        ...
                        totalFromApi = 0;
                        roomTypeTotals.forEach(function (count) { totalFromApi += count; });
                        if (totalFromApi > 0) {
                            console.log('[Elektra] Dynamic total rooms from API:', totalFromApi);
                            return [2 /*return*/, totalFromApi];
                        }
                        console.warn('[Elektra] Could not determine total rooms from API, using fallback:', TOTAL_ROOMS);
                        return [2 /*return*/, TOTAL_ROOMS];
                    case 2:
                        _a.sent();
                        console.warn('[Elektra] getTotalRooms error, using fallback:', TOTAL_ROOMS);
                        return [2 /*return*/, TOTAL_ROOMS];
                    case 3: return [2 /*return*/];
                }
            });
