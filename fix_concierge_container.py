...
try:
    ...
    # 3. Rebuild Next.js
    results.append("\n=== Rebuilding Next.js ===")
    out, err = run(c, f'docker exec -w /app -e DATABASE_URL="postgresql://coolify:***REDACTED_DB_PASSWORD***@coolify-db:5432/blue_dreams_v2" {cid} npm run build 2>&1', timeout=300)
